...
{
    $mailbox_size = Get-MailboxStatistics -Identity $mailbox | select @{name="TotalItemSizeinGB"; expression={[math]::Round( `
    ...
    $archive_size = Get-MailboxStatistics -Identity $mailbox -Archive | select @{name="TotalItemSizeinGB"; expression={[math]::Round( `
    ($_.TotalItemSize.ToString().Split("(")[1].Split(" ")[0].Replace(",","")/1GB),2)}} | select TotalItemSizeinGB -ExpandProperty TotalItemSizeinGB
